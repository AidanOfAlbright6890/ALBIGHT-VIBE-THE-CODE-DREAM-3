...
class Food:
    ...
    def drawLabel(self, font):
        label = font.render(f"{self.name} - ${self.price}", True, BLACK, WHITE)
    ...
    while True:
        for event in pygame.event.get():
            ...
            if oplaybutton.handleEvent(event):
                oCustomerAnimation.play((420, 311))
        ...
        if not oCustomerAnimation.moving and not customerAtCounter:
        ...
        messageSurface = bigFont.render(message, True, BLACK, WHITE)
        window.blit(messageSurface, (30, 20))

        if selectedItem is not None:
            infoSurface = font.render(f"Selected: {selectedItem.name}    Paid: ${moneyPaid}", True, BLACK, WHITE)
